...
export default function ObserverSidebar({ user, collapsed, setCollapsed, isMobile }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      // If no organization found in user object, fetch from observer dashboard (like settings)
      if (!organization) {
        const fetchUserOrgData = async () => {
          try {
            ...
            // Extract organization from dashboard API (overview)
            const org = response?.data?.data?.overview?.organization || '';
            setUserOrganization(org || 'Independent Observer');
          } catch (err) {
            console.error('Error fetching user organization from dashboard:', err.message);
