...
function Register() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "faculty") {
      ...
      // keep studentId and, for students, auto-fill email when appropriate
      setForm((f) => {
        const next = { ...f, studentId: value };
        // If role is student and email is empty or matches the old autogenerated pattern, update it
        try {
          const oldEmail = (f.email || '').trim();
          const generatedOld = f.studentId ? `${f.studentId}@std.kyu.ac.ug` : '';
          if (f.role === 'student' && (!oldEmail || oldEmail === generatedOld)) {
            next.email = `${value.trim()}@std.kyu.ac.ug`;
          }
        } catch (err) {
          // ignore
        }
